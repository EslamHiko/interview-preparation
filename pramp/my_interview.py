...
existss = {71:1,93:1,82:1,44:1,0:1}


smallest nonnegative integer that isnt in the array.
'''
...
function getDifferentNumber(arr):
    ...
    for i from 0 to n-1:
        ...
        while (temp < n AND arr[temp] != temp):
            swap(temp, arr[temp])
                 2,3 => [3,5,2,7]
                 temp = 3,n = 4,arr[temp] = 7
                 [7,5,2,3]
                 temp = 7,n = 4, arr[temp] = 3
                 [2,3,5,7]
...
sorted = collections.deque(el[0])
for el in range(len(arr)):
  if el > sorted[l]:
    sorted.appendleft(el)
  else:
    sorted.append(right)
'''''
