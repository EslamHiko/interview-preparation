...
def sol(arr):
    ...
    for room in arr:
        if room[0] == '+':
            if bookedMap.get(room[1:]):
                bookedMap[room[1:]] += 1
            else:
                bookedMap[room[1:]] = 1
            if maxBooked <= bookedMap[room[1:]]:
                if maxBooked == bookedMap[room[1:]]:
                    maxRoom = min(maxRoom,room[1:])
                else:
                    maxRoom = room[1:]
                maxBooked = bookedMap[room[1:]]

    return maxRoom

print(sol(["+1A", "+3E", "-1A", "+4F", "+1A", "-3E"]))
print(sol(["+1A", "+3E", "-1A", "+4F", "+1A","-4F","+4F", "-3E"]))
print(sol(["+1A", "+3E", "-1A", "+4F", "+1A","-4F","+4F", "-3E","-4F","+4F"]))
